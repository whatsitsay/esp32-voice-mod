...
/*
 * Iterative version of the delay chorus effect
...
float delay_line[N_SAMPLES];
float* prev_frame = &delay_line[0];
float* curr_frame = &delay_line[HOP_SIZE];
float prev_tx_frame[HOP_SIZE];
...
void audio_data_modification(int* txBuffer, int* rxBuffer) {
    ...
    for (int i = 0; i < HOP_SIZE; i++) {
        curr_frame[i] = rxBuffer[2 * i];
    }

    // Fill previous frame of tx buffer
    for (int i = 0; i < HOP_SIZE; i++)
    {
        prev_tx_frame[i] = txBuffer[2 * i];
    }

    // Iterate over delay line starting from midpoint ("hop size")
    // This is the starting point for the current frame
    for (int i = 0; i < HOP_SIZE; i++) {
        // Calculate given index delay
        float sample_delay_ms = BASE_DELAY_MS + DELAY_VAR_MAX_MS * sinf((2 * M_PI * LFO_FREQ_HZ * (lfo_idx + i)) / I2S_SAMPLING_FREQ_HZ);
        int sample_delay_idx  = roundf((sample_delay_ms / 1000) * I2S_SAMPLING_FREQ_HZ);
        configASSERT( sample_delay_idx <= HOP_SIZE ); // Double check not surpassing boundary of buffering

        // Output will be input + delay, each multiplied by separate gain
        // Can use negative index as current frame is located at midpoint of delay line
        // Small bit of 
        txBuffer[2 * i] = DRY_GAIN * curr_frame[i] + WET_GAIN * curr_frame[i - sample_delay_idx];
        // Copy left into right
        txBuffer[2 * i + 1] = txBuffer[2 * i];
    }

    // Move up delay line by copying latter half to beginning of address space
    memcpy(prev_frame, curr_frame, HOP_BUFFER_SIZE_B);
